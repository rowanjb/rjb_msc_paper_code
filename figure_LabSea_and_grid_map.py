...
# note that standard parallels are the parallels of correct scale
projection = ccrs.NearsidePerspective(
    central_longitude=-35,
    central_latitude=35,
    satellite_height=10005785831
)
...
# This is to ensure you don't clip any edge of the circle
# from https://stackoverflow.com/questions/73178207/how-to-plot-a-...
# map-of-a-semi-sphere-eg-northern-hemisphere-using-matplotlib-car
...
p1 = ax2.pcolormesh(
    ds.nav_lon,
    ds.nav_lat,
    ds['e1t'].isel(t=0)/1000,
    transform=ccrs.PlateCarree(),
    cmap=cm,
    rasterized=True  # Good if saving as PDF
)
...
plt.switch_backend('agg')  # From the DKRZ guide
...
for row in range(np.shape(z2)[0])[1:-1]:
    for col in range(np.shape(z2)[1])[1:-1]:
        if z2[row, col] == 1:
            if z2[row-1, col] == 1:
                z3[row-1, col] = z3[row-1, col] + 1
            if z2[row+1, col] == 1:
                z3[row+1, col] = z3[row+1, col] + 1
            if z2[row, col-1] == 1:
                z3[row, col-1] = z3[row, col-1] + 1
            if z2[row, col+1] == 1:
                z3[row, col+1] = z3[row, col+1] + 1
            if z2[row+1, col+1] == 1:
                z3[row+1, col+1] = z3[row+1, col+1] + 1
            if z2[row+1, col-1] == 1:
                z3[row+1, col-1] = z3[row+1, col-1] + 1
            if z2[row-1, col+1] == 1:
                z3[row-1, col+1] = z3[row-1, col+1] + 1
...
plt.savefig('figure_LabSea_and_grid_map.svg', dpi=600)
plt.clf()
