...
def stability_figure():
    ...
    fig, [ax1, ax2, ax3, ax4] = plt.subplots(
        nrows=1, ncols=4, figsize=(19*cm, 15*cm))
    ...
    def plot_profiles(ds1, ds2, ax, letter, title):
        ds_diff = ds1-ds2
        ds_diff['N2_temp'].plot(y='deptht', ax=ax, c=c1)
        ds_diff['N2_salt'].plot(y='deptht', ax=ax, c=c2)
        ds_diff['N2'].plot(y='deptht', ax=ax, c=c3)
        ax.invert_yaxis()
        ax.set_ylim(1000, 0)
        ax.set_ylabel('')
        ax.set_xlabel('')
    ...
    # Opening the files
    def op_stab(run):
        ds = xr.open_dataset('test_stability_LS3000_'+run+'.nc')
    ...
    #    ax.set_xlim(-1e-5, 2.7e-5)

    plot_profiles(op_stab("EPM151"), op_stab("EPM157"), ax1, 'a', 'EPM151-EPM157')
    plot_profiles(op_stab("EPM155"), op_stab("EPM151"), ax2, 'b', 'EPM155-EPM151')
    plot_profiles(op_stab("EPM152"), op_stab("EPM158"), ax3, 'c', 'EPM152-EPM158')
    plot_profiles(op_stab("EPM156"), op_stab("EPM152"), ax4, 'd', 'EPM156-EPM152')
