...
def plot_MLDs_body_figure():
    ...
    ax1.set_title(
        r'10-yr winter mean MLD',#, $\overline{MLD}_{DJFMA}$',
    ...
    def plt_mini_map(run, title, ax, letter):
        ...
        gl = ax.gridlines(draw_labels=True, dms=False, x_inline=False, y_inline=False, linewidth=0.5)
        ...
        if run=='EPM156': 
            gl.bottom_labels=True
            gl.right_labels=True
        gl.ylocator = mticker.FixedLocator([50, 55, 60, 65, 70, 75, 80]) #[52.5, 55, 57.5, 60, 62.5, 65, 67.5, 70])  #
    ...
    axins.set_xlabel(r'10-yr mean of yearly max MLD ($m$)',size=12,labelpad=8)#,loc='left',labelpad=8)
    axins.xaxis.set_label_position('top') 
    #axins.set_ylabel('$MLD$\n($m$)',rotation=0,fontdict={'fontsize':12})
